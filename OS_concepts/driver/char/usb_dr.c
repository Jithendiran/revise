...
#define USB_SKEL_VENDOR_ID	0x2717
#define USB_SKEL_PRODUCT_ID	0xff40
...
static int skel_open(struct inode *inode, struct file *file)
{
	...
	printk(KERN_INFO "SKL OPEN done...\n");
...
{
	printk(KERN_INFO "SKL READ Start...\n");
	struct usb_skel *dev;
	int retval = 0;
	...
	retval = usb_bulk_msg(dev->udev, /*device*/
			      ...
			      (int *)&count, /* data have been copied*/ 
				  HZ*10 /* wait time out*/
				  );
...
{
	printk(KERN_INFO "SKL WRITE Call back start...\n");
...
static ssize_t skel_write(struct file *file, const char __user *user_buffer, size_t count, loff_t *ppos)
{
	...
	printk(KERN_INFO "SKL write end...\n");

exit:
	return count;

error:
	printk(KERN_INFO "SKL write error...\n");
...
{
	printk(KERN_INFO "SKL disconnect ... \n");
	struct usb_skel *dev;
